...
public class EnemyManager {
    ...
    public Enemy getEnemyAt(int mouseX, int mouseY) {
        ...
        for (Enemy e : enemies) {
            double distance = Math.sqrt(Math.pow(e.getX() - mouseX, 2) + Math.pow(e.getY() - mouseY, 2));
    ...
    private void loadEnemyImgs() {
        for (int i = 0; i < 14; i++) {
            enemyImgs[i] = LoadSave.getSpriteByName(Constants.Monsters.getSpriteName(i),
                    Constants.Monsters.getSpriteType(i));
        }
    }
